...
export default function Grainient({
  ...
  useEffect(() => {
    ...
    const tryStart = () => {
      if (isVisible && isPageVisible && !motionQuery.matches && raf === 0) {
        raf = requestAnimationFrame(loop);
      }
    };
    const tryStop = () => {
    ...
    const onMotionChange = () => {
      if (motionQuery.matches) {
        tryStop();
        // Render one static frame so the backdrop does not go blank.
  ...
  return <div ref={containerRef} className={`grainient ${className}`.trim()} aria-hidden="true" />;
}
