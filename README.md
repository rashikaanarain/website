...
2. **Hero** — "How can we 10x access to justice?" + a "bulletin board" panel showing live work (Big Bail Bash, wage recovery, "bring a stuck problem"). Behind it, a **muted, auto-looping video** plays as background — a native `<video>` element in `.hero-video` (autoplay/muted/loop/playsinline), under a dark gradient (`.hero-overlay`) so the light hero text stays legible. The source is OpenNyAI's own hosted clip on its DigitalOcean CDN (`onmain.blr1.cdn.digitaloceanspaces.com/mainsite/main-on-hero-video-720-02.mp4`, the same file the Framer prototype uses). To change it, edit the `<source src>`. **Rights note:** the footage is snippets from a third-party documentary — confirm licensing before a public launch.
